...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"RfyS":[function(require,module,exports) {
var define;
...
var _jquery = _interopRequireDefault(require("../plugins/jquery-3.5.1.min"));
...
var data = {
  ...
  tipBoxSpaceTop: 300,
  // 临时悬浮通知窗向下滚动的距离 (px)
  headDomClassName: '.headBox',
  // 头部的选择器
  menuBtns: '.headBox button[targetclassname]',
  // 菜单按钮们的选择器
...
(0, _jquery.default)(function () {
  // 设置头部高度
  setHeaderBoxHeight(); // 绑定头部的点击事件 + 监听window滚动，设置头部按钮高亮

  setScrollBtn();
}); // 设置头部高度

function setHeaderBoxHeight() {
  data.headerBoxHeight = Math.ceil((0, _jquery.default)(data.headDomClassName).height() || 0);
} // 绑定头部的点击事件 + 监听window滚动，设置头部按钮高亮


function setScrollBtn() {
  data.doms = document.querySelectorAll(data.menuBtns);
  data.$headBox = (0, _jquery.default)(data.headDomClassName);
  (0, _jquery.default)(window).off('scroll', windowScroll);
  (0, _jquery.default)(window).on('scroll', windowScroll);
  if (!data.doms || !data.doms.length) return;
  data.topSpaceArr = []; // 存放着按钮和目标dom位置对象的数组

  var _loop = function _loop(i) {
    var dom = data.doms[i],
        $dom = (0, _jquery.default)(dom);
    var targetClassName = dom.getAttribute('targetclassname');
    if (!targetClassName) return "continue";
    var targetDom = document.querySelector('.' + targetClassName);
    if (!targetDom) return "continue";
    var widthOut = $dom.outerWidth();
    var spaceLeft = $dom.position().left;
    ...
    var targetclassname = dom.getAttribute('targetclassname');
    var inDoms = (0, _jquery.default)("[targetclassname=".concat(targetclassname, "]"));

    for (var _i = 0; _i < inDoms.length; _i++) {
      inDoms[_i].onclick = judge;
    } // 绑定点击后跳转事件


    function judge() {
      var targetClassName = this.getAttribute('targetclassname');
      if (!targetClassName) return;
      var targetDom = document.querySelector('.' + targetClassName);
      if (!targetDom) return;
      var clickScrollBtnId = +new Date();
    ...
    for (var i = 0; i < data.topSpaceArr.length; i++) {
      ...
      if (targetSpace <= scrollTop) {
        ...
        if (!data.topSpaceArr[i + 1] || data.topSpaceArr[i + 1].targetSpace > scrollTop) {
          ...
          var $flagLine = (0, _jquery.default)(data.lineDomClassName);
          ...
          if ($flagLine.attr('data_width') != obj.width || $flagLine.attr('data_left') != obj.spaceLeft) {
            ...
            $flagLine.attr({
              data_width: obj.width,
              data_left: obj.spaceLeft
            });
          }
...
function scrollSmoothTo(position, doms, btnDom, clickScrollBtnId, obj) {
  if (clickScrollBtnId !== data.clickScrollBtnId) return; // 先点亮按钮
  ...
  var step = function step() {
    if (clickScrollBtnId !== data.clickScrollBtnId) return; // 距离目标滚动距离
...
},{"../plugins/jquery-3.5.1.min":"RfyS"}]},{},["QvaY"], null)